...
public class PrefabSpawner : MonoBehaviour {
    ...
    public void SpawnObjects(GameObject prefab, int count, Vector3 offset)
    {
        ...
        if(!isOffsetRelative)
        {
            ...
            {
                Instantiate(prefab, offset * i, objectRotation, transform);
            }
        }
        else
        {
            ...
            for (int i = 0; i < count; i++)
            {
                GameObject lastSpawned = Instantiate(prefab, lastTransform.position, objectRotation * lastTransform.rotation, transform);
